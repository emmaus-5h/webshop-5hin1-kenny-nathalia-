...
function getProducts(request, response) {
  ...
  let data = []
  const sqlOpdracht = db.prepare('SELECT products.id AS id, products.name AS name, products.description AS description, products.code AS code, products.price AS price, artiesten.age AS age, artiesten.country AS country, genres.name AS genre, album_release.datum AS datum, artiesten.name AS artiesten FROM products LEFT OUTER JOIN genres ON products.genres_id = genres.id LEFT OUTER JOIN album_release ON products.album_release_id = album_release.id LEFT OUTER JOIN types ON products.types_id = types.id LEFT OUTER JOIN artiesten ON products.artiesten_id = artiesten.id JOIN streaming-platforms_artiesten ON products.id = streaming-platforms_artiesten.streaming_platform_id JOIN streaming-platforms ON streaming-platforms.id = streaming-platforms_artiesten.artiesten2_id GROUP BY products.id ORDER BY artiesten_id ASC')
